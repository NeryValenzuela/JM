...
route.get("/", (req, res) => {
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_search_custumer`);
    })
    .then((fields) => {
      mssql.close();
...
route.post("/", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_create_custumer '${body.firstName}','${body.lastName}','${body.address}','${body.phone}'`);
    })
    .then((fields) => {
      mssql.close();
...
route.put("/", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_update_custumer '${body.uuidCustumer}','${body.firstName}','${body.lastName}','${body.address}','${body.phone}'`);
    })
    .then((fields) => {
      mssql.close();
...
route.delete("/:id", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_delete_custumer '${id}'`);
    })
    .then((fields) => {
      mssql.close();
...
exports.ControllerCustumer = route;
