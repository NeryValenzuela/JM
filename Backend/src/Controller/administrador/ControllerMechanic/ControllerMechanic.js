...
route.get("/", (req, res) => {
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_search_mechanic`);
    })
    .then((fields) => {
      mssql.close();
...
route.put("/", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_update_mechanic '${body.uuidMechanic}','${body.firstName}','${body.lastName}','${body.phone}','${body.checkBonus}'`);
    })
    .then((fields) => {
      mssql.close();
...
route.delete("/:id", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_delete_mechanic '${id}'`);
    })
    .then((fields) => {
      mssql.close();
...
exports.ControllerMechanic = route;
