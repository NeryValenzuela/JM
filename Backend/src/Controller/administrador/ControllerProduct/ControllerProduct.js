...
route.get("/", (req, res) => {
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_search_product`);
    })
    .then((fields) => {
      mssql.close();
...
route.post("/", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_create_product '${body.product}','${body.description}','${body.price}','${body.stock}'`);
    })
    .then((fields) => {
      mssql.close();
...
route.put("/", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_update_product '${body.uuidProduct}','${body.product}','${body.description}','${body.price}','${body.stock}'`);
    })
    .then((fields) => {
      mssql.close();
...
route.delete("/:id", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_delete_product '${id}'`);
    })
    .then((fields) => {
      mssql.close();
...
exports.ControllerProduct = route;
