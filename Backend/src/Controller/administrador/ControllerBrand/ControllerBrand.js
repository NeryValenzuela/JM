...
route.get("/", (req, res) => {
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_search_brand`);
    })
    .then((fields) => {
      mssql.close();
...
route.post("/", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_create_brand '${body.brand}'`);
    })
    .then((fields) => {
      mssql.close();
...
route.put("/", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_update_brand '${body.uuidBrand}','${body.brand}'`);
    })
    .then((fields) => {
      mssql.close();
...
route.delete("/:id", (req, res) => {
  ...
  new mssql.ConnectionPool(connect.config)
    .connect()
    .then((pool) => {
      return pool.request().query(`EXEC sp_delete_brand '${id}'`);
    })
    .then((fields) => {
      mssql.close();
